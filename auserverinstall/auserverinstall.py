...
#update software; stop any servers that are running
...
#install and configure aserver
aserver.pull()
aserver.install()
subprocess.call(['npm','install','apidoc','-g'], cwd = aserver.name)
subprocess.call(['apidoc','-i','routes/','-o','apidoc/'], cwd = aserver.name)
aserver.configure() #comment this line out if you already have a config.json that is properly configured and encoded

#repeat the process for userver
userver.pull()
userver.install()
subprocess.call(['npm','install','apidoc','-g'], cwd = userver.name)
subprocess.call(['apidoc','-i','routes/','-o','apidoc/'], cwd = userver.name)
userver.configure() #comment this line out if you already have a config.json that is properly configured and encoded

subprocess.call(['pm2','start','process.json'])

#commands for starting the servers
#subprocess.call(['node','app.js'], cwd = aserver.name)
#subprocess.call(['node','app.js'], cwd = userver.name)

sleep(1.5)

#command for testing a relevant URL
