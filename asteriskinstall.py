...
#download Asterisk
subprocess.call(['wget', 'http://downloads.asterisk.org/pub/telephony/asterisk/releases/asterisk-14.2.1.tar.gz'], cwd = '/usr/src')
subprocess.call(['tar', '-zxf', 'asterisk-14.2.1.tar.gz'], cwd = '/usr/src')

#install pre-requisites
subprocess.call(['./contrib/scripts/install_prereq'], cwd = '/usr/src/asterisk-14.2.1')

#install PJSIP and asterisk

subprocess.call(['./configure', '--with-pjproject-bundled'], cwd = '/usr/src/asterisk-14.2.1')
subprocess.call(['make', '&&', 'make', 'install', '&&', 'make', 'samples'], cwd = '/usr/src/asterisk-14.2.1')
subprocess.call(['make', 'config'], cwd = '/usr/src/asterisk-14.2.1')
subprocess.call(['service', 'asterisk', 'start'], cwd = '/usr/src/asterisk-14.2.1')

#add git repo domain name to known_hosts so RSA fingerprint prompt does not pause script
subprocess.call(['ssh-keyscan', 'github.com', '>>', '~/.ssh/known_hosts'], cwd = '/usr/src/asterisk-14.2.1')

# pull down confi/media files and add to /etc/asterisk and /var/lib/asterisk/sounds, respectively
subprocess.call(['git', 'clone', '$9'], cwd = '~')
